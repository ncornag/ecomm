...
export const getCatalogSyncCollection = (
  db: Db,
): Collection<CatalogSyncDAO> => {
...
export interface ICatalogSyncRepository {
  ...
  findOne: (
    id: string,
    version?: number,
  ) => Promise<Result<CatalogSyncDAO, AppError>>;
  find: (
    query: any,
    options?: any,
  ) => Promise<Result<CatalogSyncDAO[], AppError>>;
...
export class CatalogSyncRepository
  implements ICatalogSyncRepository, ITreeRepo<CatalogSyncDAO>
{
  ...
  async create(
    ...
    if (!result || result.insertedId == '')
      ...
      return new Err(
        new AppError(ErrorCode.BAD_REQUEST, `Can't save catalogSync [${_id}]`),
      );
  ...
  async updateOne(
    ...
    if (result.modifiedCount != 1)
      return new Err(
        new AppError(ErrorCode.BAD_REQUEST, `Can't update catalogSync.`),
      );
    return new Ok({});
  }
    ...
    if (!entity) {
      return new Err(
        new AppError(
          ...
          `Can't find catalogSync with id [${id}]`,
        ),
      );
    }
