...
import { PromotionService } from '../promotion.svc';
...
export class PromotionsEngine {
  ...
  async evaluateWhen(when: When, facts: any, bindings: any) {
    ...
    for (const [key, value] of Object.entries(when)) {
      ...
      if (
        ...
        (typeof expressionResult === 'boolean' && expressionResult !== true)
      ) {
        if (this.server.logger.isLevelEnabled('debug'))
          this.server.log.debug(
            gray(
              `    ${key}: ${value} = ${expressionResult?.sku ? expressionResult.sku : expressionResult}`,
            ),
          );
      } else {
        if (this.server.logger.isLevelEnabled('debug'))
          this.server.log.debug(
            green(
              `    ${key}: ${reset(value)} = ${expressionResult?.sku ? expressionResult.sku : expressionResult}`,
            ),
          );
      }
      if (
  ...
  async run(facts: any, promotionId?: string): Promise<Result<any, AppError>> {
    ...
    const productsInCart = facts.items.reduce(
    ...
    for await (const promotion of promotions) {
      ...
      do {
        if (this.server.logger.isLevelEnabled('debug'))
          this.server.log.debug(
            yellow(
              `  Pass ${executions + 1}/${promotion.times ? promotion.times : '∞'}`,
            ),
          );
    ...
    const perMs = ((1000000 * promotions.length) / Number(end - start)).toFixed(
      2,
    );
