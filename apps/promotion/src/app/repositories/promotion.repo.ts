...
export interface IPromotionRepository {
  ...
  findOne: (
    id: string,
    version?: number,
  ) => Promise<Result<PromotionDAO, AppError>>;
  find: (
    query: any,
    options?: any,
  ) => Promise<Result<PromotionDAO[], AppError>>;
...
export class PromotionRepository implements IPromotionRepository {
  ...
  async create(promotion: Promotion): Promise<Result<PromotionDAO, AppError>> {
    ...
    if (!result || result.insertedId == '')
      ...
      return new Err(
        new AppError(ErrorCode.BAD_REQUEST, `Can't save promotion [${_id}]`),
      );
  ...
  async updateOne(
    ...
    if (result.modifiedCount != 1)
      return new Err(
        new AppError(ErrorCode.BAD_REQUEST, `Can't update promotion.`),
      );
    return new Ok({});
  }
    ...
    if (!entity) {
      return new Err(
        new AppError(
          ...
          `Can't find promotion with id [${id}]`,
        ),
      );
    }
