...
class ProductCreator {
  ...
  private createProduct(projectId: string, catalog: string): any {
    return {
      ...
      categories: [this.categories[this.randomIntFromInterval(1, this.categories.length - 1)]]
      //priceMoes: this.ct.PriceMode.EMBEDDED
    };
  }

  private createVariant(projectId: string, parent: any, pricesPerVariant: number, productsToInsert: number): any {
    const sku = fakerEN.commerce.isbn(13);
    let order = 1;
    const prices = Array.from({ length: pricesPerVariant }, (_, i) =>
      this.createPrice(projectId, parent.catalogId, sku, order++, this.predicatesOrder)
    );
    prices.push(this.createPrice(projectId, parent.catalogId, sku, order++, [[]]));
    return [
      {
        type: 'variant',
        parent: parent.id,
  ...
  private createPrice(projectId: string, catalog: string, sku: string, order: number, predicatesOrder: any): any {
    ...
    return {
      ...
      predicates: Array.from({ length: predicatesOrder.length }, (_, i) => {
        const predicates = predicatesOrder[i].reduce(
          (acc: any, curr: any) => {
            acc[curr] = [
              this.predicateValues[curr][this.randomIntFromInterval(0, this.predicateValues[curr].length - 1)]
            ];
  ...
  public async createProducts(productsToInsert = 1, variantsPerProduct = 1, pricesPerVariant = 1) {
      ...
      arr.map(async (elem) => {
        ...
        for (let j = 0; j < variantsPerProduct; j++) {
          ...
          for (let k = 0; k < prices.length - 1; k++) {
            prices[k].sku = variantsResult.value.sku;
...
const argv = [process.argv[0], 'nx run createProducts:run --args="', ...(process.argv[2] || '').split(' ')];
...
async function main() {
  try {
    await productCreator.createProducts(flags.products, flags.variants, flags.prices);
    console.log('Done!');
    process.exit(0);
  } catch (e) {
    console.error(e);
